...
This release fixes a large batch of options that were silently ignored or
could only ever be turned on across several modules (``backup``, ``restore``,
``install_script``, ``availability_group``, ``login``, ``sa``), replaces
fragile idempotency comparisons with a shared helper, and makes several
modules re-read live server state for their returned ``data`` instead of
echoing back the requested values. See the Breaking Changes section below
before upgrading, since some previously-ignored options now take effect.
